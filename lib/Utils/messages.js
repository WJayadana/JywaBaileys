...
const generateWAMessageContent = async (message, options) => {
    ...
    if (hasNonNullishProperty(message, "buttonReply")) {
        ...
        if (hasNonNullishProperty(interactiveMessage, "header")) {
            branding.header = interactiveMessage.header
        } else {
            const header = {
                title: interactiveMessage.title || null,
                subtitle: interactiveMessage.subtitle || null,
                hasMediaAttachment: !!(interactiveMessage.image || interactiveMessage.video || interactiveMessage.document),
            }

            if (interactiveMessage.image || interactiveMessage.video || interactiveMessage.document) {
                const media = await prepareWAMessageMedia(interactiveMessage, options)
        ...
        if (hasNonNullishProperty(interactiveMessage, "body")) {
            branding.body = interactiveMessage.body
        } else if (hasNonNullishProperty(interactiveMessage, "text")) {
            branding.body = { text: interactiveMessage.text }
        }

        if (hasNonNullishProperty(interactiveMessage, "footer")) {
            branding.footer = interactiveMessage.footer
        }
